...
`.gitea/workflows/deploy.yaml` baut und deployt bei jedem Push auf `main`: der Runner
(Container mit Zugriff auf den Host-Docker) macht `actions/checkout`, dann
`docker compose -f docker-compose-prod.yml build` + `up -d`, und prüft per Smoke-Test,
